...
class SMPL2Mesh:
    def __init__(self, bm_path, bm_type='smplh'):
        ...
        self.f = self.models['male']['f'] # face列表，[13776，3] 很多，这个方法要求顶点数量around 5K左右最好
        self.bm_type = bm_type
        simplify_path = f"{bm_path}/simplify.npz"

        # since the subsequent procedure will then crash,
        #   it's not an optional choice (but be puzzlable).
        assert os.path.exists(simplify_path)
